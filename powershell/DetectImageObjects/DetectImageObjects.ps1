...
#Requires -Modules @{ModuleName='AWS.Tools.Rekognition';ModuleVersion='4.1.8.0'}
...
foreach ($record in $LambdaInput.Records) {
    ...
    if ($imageExtensions.Contains($ext)) {
        ...
        $labels = (Find-REKLabel -ImageBucket $bucket -ImageName $key -MinConfidence $confidence).Labels  | Select-Object -First 10
